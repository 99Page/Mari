...
private extension AlertState where Action == TabFeature.Action.Alert {
    static let eula = AlertState {
        ...
        TextState(
            """
            1. 본 서비스는 불쾌, 혐오, 폭력적, 성적, 불법 또는 기타 부적절한 콘텐츠를 절대 허용하지 않습니다.
            2. 사용자는 다른 사용자의 콘텐츠를 신고하거나 해당 사용자를 차단할 수 있습니다.
            3. 신고되거나 정책을 위반한 콘텐츠 및 사용자는 운영팀이 신속히 검토하여, 해당 콘텐츠 삭제, 계정 일시/영구 정지 등 적절한 조치를 취합니다.
            4. 모든 이용자는 서비스 이용 전 전체 이용 약관(EULA)에 동의해야 하며, 동의하지 않는 경우 서비스 이용이 제한됩니다.
            """
        )
    }
